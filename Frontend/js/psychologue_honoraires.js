...
async function saveTarif() {
    ...
    try {
        const result = await doctorAPI.updateTarif(parsedTarif);
        showToast('Tarif mis à jour: ' + parsedTarif + ' DA', 'success');
...
function formatDate(dateString) {
    ...
    return date.toLocaleDateString('fr-FR', { day: 'numeric', month: '2-digit', year: 'numeric' });
}
...
function showToast(message, type = 'info') {
    ...
    toast.style.cssText = `
        ...
        background: ${type === 'success' ? '#44AA99' : type === 'error' ? '#e74c3c' : '#666'};
    `;
